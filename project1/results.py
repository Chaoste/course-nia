...
def get_rls_go():
    title = r'Random Local Search ($greater\_only=True$)'
    labels = ["one_max", "leading_ones"]
    all_runtimes = np.array([
        [
            [  65, 108,  89,  75,  69,  83,  83,  59,  74,  73],
            [ 198, 119,  71,  94, 164, 236, 157, 256, 189, 168],
            [ 216, 379, 252, 349, 270, 225, 301, 487, 243, 170],
            [ 417, 549, 458, 212, 510, 372, 432, 310, 526, 384],
            [ 701, 819, 401, 439, 454, 600, 545, 570, 496, 610],
            [ 1109,  775,  827,  797,  698,  617,  678,  822,  512,  570],
            [ 1111,  862,  674,  756,  718,  908,  844,  745,  843,  726],
        ],
        [
            [ 127, 176, 162, 323, 263, 236, 423, 309, 370, 264],
            [ 1293,  918, 1079,  944, 1363, 1194, 1120, 1809, 1009, 1226],
            [ 2195, 3493, 2305, 2818, 2909, 2210, 2678, 3146, 3729, 3523],
            [ 3855, 5626, 5311, 3705, 6644, 5356, 5866, 4854, 5100, 6678],
            [  9296,  6423,  7737,  6870,  7697,  6978,  9531,  8286, 10081, 6128],
            [ 10989,  9993, 10781, 10382, 12119,  9760,  8713,  8221, 12894, 9999],
            [ 14819, 14022, 13459, 11834, 21068, 14480, 17024, 13512, 17872, 16443],
        ],
    ])
    return title, labels, all_runtimes

def get_one_one_go():
    title = r'(1+1)-EA ($greater\_only=True$)'
    labels = ["one_max", "leading_ones"]
    all_runtimes = np.array([
        [
            [ 146, 103,  61, 186, 170,  94, 147, 130, 299, 166],
            [ 222, 347, 373, 424, 807, 333, 336, 608, 267, 426],
            [  695,  564, 1135,  795,  840,  574,  762, 1392, 1499,  683],
            [  682,  744, 1352,  705,  992, 1358,  634,  772, 1243,  753],
            [ 1347,  754, 1152, 1235,  617, 1479, 1114, 1134, 1737,  994],
            [ 1398, 1905, 1104, 1416, 1102, 2026, 1535, 1462, 1216, 1354],
            [ 1740, 1762, 2363, 1666, 3052, 1877, 1760, 2102, 2430, 1343],
        ],
        [

            [ 268, 660, 732, 540, 480, 423, 470, 537, 607, 307],
            [ 2585, 1870, 2635, 2180, 1828, 1305, 2707, 2192, 2551, 1719],
            [ 5238, 4567, 3250, 3399, 3929, 4367, 5357, 3573, 4552, 6125],
            [  6678,  7895,  8542,  8526,  6422,  7372, 10129,  8345,  7527, 8978],
            [ 11146, 12594, 18626, 16216, 14090, 11110, 14346, 11258, 12501, 11522],
            [ 17130, 19618, 22223, 17365, 23195, 21073, 19244, 21861, 15179, 21433],
            [ 26172, 33375, 25956, 21541, 25596, 25682, 30268, 28565, 30947, 28446],
        ],
    ])
    return title, labels, all_runtimes

def get_one_lambda_2_go():
    title = r'(1+$\lambda$)-EA  [$\lambda = 2$] ($greater\_only=True$)'
    labels = ["one_max", "leading_ones"]
    all_runtimes = np.array([
        [
            [  84, 106, 121,  50,  76, 153,  93, 116,  66,  93],
            [ 184, 235, 202, 193, 193, 257, 367, 214, 269, 136],
            [ 243, 261, 404, 642, 325, 420, 580, 309, 367, 385],
            [ 467, 287, 344, 418, 622, 682, 561, 476, 555, 653],
            [  785,  544,  685,  625, 1285,  537,  775,  510,  469,  495],
            [ 1162,  934,  528,  951, 1073,  857,  764,  801,  582,  603],
            [  950,  776, 1259, 1283,  968,  981, 1185, 1414,  993, 1420],
        ],
        [
            [ 344, 208, 328, 314, 170, 192, 424, 213, 146, 332],
            [ 1058,  913,  757,  841, 1527,  938, 1199, 1312, 1264, 1138],
            [ 3470, 2372, 2047, 2360, 2970, 2593, 2756, 3474, 3911, 2830],
            [ 4363, 3894, 4263, 3769, 3994, 4644, 4386, 5683, 4121, 4959],
            [ 5619, 8396, 4724, 6165, 4939, 6816, 7447, 7082, 7761, 6327],
            [  7066,  7863,  9173,  8660,  9895,  8602,  8754, 11448, 11716, 10648],
            [ 12432, 12778, 12132, 14822, 15297, 15116, 12091, 13630, 10037, 10938],
        ],
    ])
    return title, labels, all_runtimes

def get_one_lambda_10_go():
    title = r'(1+$\lambda$)-EA  [$\lambda = 10$] ($greater\_only=True$)'
    labels = ["one_max", "leading_ones"]
    all_runtimes = np.array([
        [
            [ 40, 34, 17, 23, 20, 25, 19, 25, 23, 23],
            [  67,  40,  49, 103,  47,  48,  46,  59,  43,  53],
            [  90,  58,  72,  84,  69,  54,  68,  68, 100,  92],
            [ 146, 162, 111,  73, 108,  83, 157, 141, 156, 172],
            [ 148, 219, 180, 129, 190, 255, 139, 144, 250, 163],
            [ 226, 177, 185, 265, 246, 264, 192, 269, 231, 191],
            [ 257, 311, 180, 270, 192, 211, 268, 233, 285, 259],
        ],
        [

            [ 42, 58, 50, 64, 57, 74, 41, 30, 46, 32],
            [ 317, 228, 309, 193, 248, 201, 143, 158, 309, 211],
            [ 448, 298, 492, 559, 620, 464, 381, 695, 345, 557],
            [  764,  748,  780,  684, 1139,  598,  678,  784,  762,  964],
            [ 1356, 1409, 1125, 1325, 1107, 1175, 1258, 1729, 1742, 1198],
            [ 2232, 2324, 2073, 2632, 2321, 2104, 2307, 1740, 1817, 1798],
            [ 3774, 3272, 2678, 3225, 2612, 2801, 2203, 2595, 2901, 1843],
        ],
    ])
    return title, labels, all_runtimes

def get_one_lambda_50_go():
    title = r'(1+$\lambda$)-EA  [$\lambda = 50$] ($greater\_only=True$)'
    labels = ["one_max", "leading_ones"]
    all_runtimes = np.array([
        [
            [ 12,  9, 10, 15,  9, 11,  9, 11,  8,  6],
            [ 22, 24, 22, 15, 24, 19, 19, 18, 25, 21],
            [ 42, 36, 31, 37, 29, 51, 30, 32, 40, 30],
            [ 50, 38, 44, 40, 40, 45, 63, 36, 46, 55],
            [ 55, 51, 40, 49, 68, 61, 67, 76, 72, 64],
            [ 65, 67, 69, 85, 57, 65, 58, 61, 77, 68],
            [ 121,  83,  72,  68,  87,  75,  78, 104,  94,  88],
        ],
        [
            [ 17, 13, 19, 11, 17, 15, 14, 16, 21, 17],
            [ 56, 42, 53, 54, 56, 78, 68, 56, 50, 35],
            [ 104, 118, 120,  94, 143, 108, 100,  96,  77,  95],
            [ 231, 153, 161, 182, 183, 243, 215, 220, 226, 207],
            [ 311, 304, 269, 347, 257, 253, 313, 387, 197, 357],
            [ 453, 447, 382, 453, 415, 359, 396, 510, 337, 380],
            [ 531, 711, 580, 464, 579, 648, 480, 578, 645, 538],
