...
def get_file_info(tsp_name):
    ...
    while ':' in lines[i]:
        ...
        problem.append((key.upper().strip(), value.strip()))
        i += 1
...
def parse_distances(problem):
    if 'EDGE_WEIGHT_FORMAT' in problem and problem['EDGE_WEIGHT_FORMAT'] is 'LOWER_DIAG_ROW':
        return parse_lower_diag_row(problem)
    else:
        raise 'Unsupported tsp data format'


def get_cities_distances(tsp_name):
    problem = get_file_info(tsp_name)
    problem['AM'] = parse_distances(problem)
    return problem
...
"""
def HeadlessAppities(list,dimension):
